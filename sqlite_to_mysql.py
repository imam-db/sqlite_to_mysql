...
parser.add_argument(
    "--sqlite-db", required=True, help="Path to the SQLite database file"
)
...
try:
    mysql_engine = sa.create_engine(
        f"mysql+mysqlconnector://{args.mysql_user}:{args.mysql_password}@{args.mysql_host}:{args.mysql_port}/{args.mysql_database}"
    )
